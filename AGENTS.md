# AGENTS.md — Spring Boot + MyBatis CRUD API（JUnit テスト教材用）
...
- **テスト一式**：
  ...
  - 統合（`@SpringBootTest` + TestRestTemplate による API テスト）
...
> **方針**：H2 データベース で実装、ユニットテスト、統合テスト実施（Oracle 対応は後課題）。SQL はできるだけ ANSI 準拠で記述
